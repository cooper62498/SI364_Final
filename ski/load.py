...
for row in [dict(row) for row in cur.fetchall()]:
    ...
    try:
        ...
        M = Mountain(name=row['MountainName'],state_name=S,
                    url=row['URL'],status=row['status'],new_snow=row['NewSnow'],
                    current_weather=row['CurrentWeather'],snowpack=row['Snowpack'],
                    open_percent=row['OpenPercent'],trails=row['Trails'],
                    lifts=row['Lifts'],base=row['Base'],conditions=row['Conditions']
                    )
        # print(M)
        # print('fail here')
        # M.save()
